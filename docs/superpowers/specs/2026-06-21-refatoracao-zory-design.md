...
- 2026-06-21: Phase 4 done — shared base at groups/_context (perms fixed a+rX) mounted RO at /workspace/extra/context (allowlist-accepted, no host restart). Persona path updated /mnt/context→/workspace/extra/context. Verified in real container (uid 1000 reads, RO enforced). NOTE: not /mnt/context — additionalMounts are forced under /workspace/extra/.
- 2026-06-21: Phase 5 done — internal tests green (real container: mount RO+readable, persona+skill loaded, config materialized, IG on Caio via native OAuth, wiki byte-identical). Mount at /workspace/extra/context (not /mnt/context — additionalMounts forced under /workspace/extra/).
- 2026-06-21: qmd follow-up — investigated bringing qmd up; ~260s/query on 2-core CPU (mandatory reranker, no lex-only MCP mode). Removed qmd MCP from all 3 agents (Zory/Caio/Lili), uninstalled CLI, reclaimed 610MB. Wiki/conversation search is file-based (grep). Zory persona Query op updated to grep.
