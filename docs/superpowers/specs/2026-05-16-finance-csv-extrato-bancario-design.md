...
Levis today only writes line-by-line through chat (confirmation card per item). Importing 30–80 lines from a bank CSV that way is unusable. The user wants to forward a CSV (BTG / Inter / Hotmart) as a Telegram attachment and have Levis:
...
The chat and CSV flows must coexist (hybrid use case). Idempotency across re-imports is non-negotiable — finance is the wrong domain for "sometimes duplicates".
...
├── bin/finance-csv                  CLI entrypoint (Node/TS, transpiled)
├── parsers/
│   ├── btg.ts                       detects BTG header, parses semicolon CSV
│   ├── inter.ts                     detects Inter header, handles Latin-1
│   └── hotmart.ts                   detects Hotmart header, receivables-only
├── reconcile.ts                     deterministic match against sheet dump
├── classify.ts                      cache lookup with normalization
└── __tests__/
    ├── parsers.test.ts              snapshot tests per bank + edge cases
    ├── reconcile.test.ts            bucket assertions per scenario
    └── classify.test.ts             cache hit/miss tests
```

The skill follows the `pdf-reader` precedent (`container/skills/pdf-reader/`) — declared `allowed-tools: Bash(finance-csv:*)`, called by the agent via `Bash`. Loaded into agent containers at runtime via the existing container skill loader.
...
The Telegram attachment handler must save uploaded CSVs (mime type `text/csv` or `.csv` extension) to `groups/<group>/imports/inbox/`. This is a small extension to the existing attachment handler — the image handler already saves to a per-group path; this mirrors that.
...
`linha_id` is deterministic (`{banco}-{data}-{seq}`) — re-parsing the same CSV produces identical IDs, enabling the `skipped_reimport` bucket. When the bank provides a stable `banco_tx_id` (BTG, Hotmart), it becomes the secondary match key. Inter doesn't, so `banco_tx_id` is null there and matching falls back to data+valor+descricao.

### Per-bank quirks the parsers absorb

| | BTG | Inter | Hotmart |
|---|---|---|---|
| Encoding | UTF-8 | Latin-1 | UTF-8 |
| Separator | `;` | `,` | `,` |
| Sign | column `tipo` (C/D) | sign on value (`-80,00`) | receitas-only |
| Date format | `dd/mm/yyyy` | `dd/mm/yyyy` | ISO |
| Stable tx_id | yes | no | yes |
| meio_pagamento_hint | extracted from `descricao` keywords | idem | always null |

Auto-detection reads the first line and matches against each parser's header regex. On no match, the binary exits non-zero with `unknown bank — header: <first 200 chars>`. Manual override: `finance-csv parse <file> --bank btg`.
...
### Lookup ordering (in `classify.ts`)

1. **Exact normalized match** of `descricao_raw` → confidence 0.95
2. **Substring match** (descricao_raw contains any `patterns[].match`) → confidence 0.80
3. **No match** → return `null`; agent classifies via IA + reads doc canônico's "regras de classificação" → confidence 0.60
...
| **Unknown bank** | Parser exits non-zero with header dump. Agent offers fallback: "Quer que eu trate como Inter (formato genérico)?" |
...
| Layer | What | Where |
|---|---|---|
| Parsers | 3 fixture CSVs per bank + 1 edge case each (encoding, estorno, transferência interna) | `container/skills/finance-csv/__tests__/parsers.test.ts` — snapshot test of canonical JSON output |
...
- `container/skills/finance-csv/bin/finance-csv`
- `container/skills/finance-csv/parsers/{btg,inter,hotmart}.ts`
- `container/skills/finance-csv/reconcile.ts`
- `container/skills/finance-csv/classify.ts`
- `container/skills/finance-csv/__tests__/{parsers,reconcile,classify}.test.ts`
