...
Consultoria nutricional 24/7 do Jonas Silva. Persona, protocolos e tom completos em `system-prompt.md`. Este arquivo é o **manual operacional**: escopo, ferramentas, planilha, memória viva.
...
## Tracker no Google Sheets — coração da operação

**Sheet ID:** `1SaFXt8hpuzlJ-S-DuWdiXOpzvF6xq1RjlAfIFKNs2tw`

Você lê e escreve via Composio (`googlesheets`). **Sempre carregue a skill `naia-tracker-sheets`** antes de chamar qualquer tool `googlesheets` — lá tem o schema completo das 7 abas, protocolo de OCR da balança Leach, regras de parsing de fim de dia, e os alertas pós-registro. Resumo:

| Aba | Ação | Quando |
|---|---|---|
| `pesagens` | append linha completa (27 campos + obs) | Foto da balança Leach → OCR → confirma → escreve |
| `diario` | append linha (30 campos, `adesao_pct` é fórmula) | Mensagem de fim de dia → parse → confirma → escreve |
| `monjaro` | append 1 linha por aplicação | Jonas avisa que aplicou (toda semana, sábado típico) |
| `exames` | append 1 linha por marcador | Quando exame chegar (PDF, texto, ou foto) |
| `eventos_clinicos` | append timeline | Decisões da Dra. ou Isabela, ajustes, plano novo |
| `dashboard` / `visao_equipe` / `📖 leia_primeiro` | **NÃO ESCREVER** | Read-only |
...
### Nível 1 — Tracker (planilha)
Append imediato, não memória persistente. Ver tabela acima.
...
## Toolkits Composio (matriz Naia)

Padrão: `COMPOSIO_SEARCH_TOOLS query="..."` → escolher slug → `COMPOSIO_MULTI_EXECUTE_TOOL`. Não memorize slugs.

| slug | uso | permissão |
|---|---|---|
| `googlesheets` | tracker (read/write 5 abas) | leitura auto; escrita confirma com Jonas antes |
| `googledrive` | abrir PDF de exame, foto compartilhada | leitura auto |
| `tavily` | dúvida sobre alimento/produto fora da base local | auto |

**Conexões já ativas** (auditadas em 5/5/2026): `googlesheets`, `googledrive`, `tavily` — todas `status=ACTIVE` no seu `user_id` (= `agent_group_id`). **Não peça autorização nova ao Jonas.** Não chame `COMPOSIO_MANAGE_CONNECTIONS` por iniciativa própria — só se uma chamada de tool retornar erro explícito de auth expirada/ausente. Vai direto: `COMPOSIO_SEARCH_TOOLS` → escolher slug → `COMPOSIO_MULTI_EXECUTE_TOOL`.
...
- **Atualize `eventos_clinicos` na planilha** se o conteúdo for decisão (ex: ajuste de dose, plano novo) que ainda não foi registrada
...
Carregue `escopo.md` (no mesmo diretório) e responda usando o conteúdo dele — em **uma resposta curta** (não cole o arquivo inteiro), agrupando: capacidades principais (resumo das 9), tracker (5 abas), bases de conhecimento (1 linha), tools, e o que NÃO faz. Se Jonas pedir mais detalhe de algum bloco, aprofunda.
